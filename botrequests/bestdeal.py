...
def get_city(message: telebot.types.Message) -> None:
    ...
    if location['suggestions'][0]['entities']:
        ...
        bot.send_message(message.chat.id, 'Введите максимальную цену проживания (USD)')
...
def get_max_price(message: telebot.types.Message) -> None:
    """
    Ввод максимальной цены проживания с проверкой корректности
    """
    global max_price
    max_price = message.text
    if max_price.isdigit():
        max_price = int(max_price)
        bot.send_message(message.chat.id, 'Введите максимальное расстояние от отеля до центра города (км)')
        bot.register_next_step_handler(message, get_max_distance)
    else:
        bot.send_message(message.chat.id, 'Цена проживания должна быть положительным целым числом')
        bot.register_next_step_handler(message, get_max_price)


def get_max_distance(message: telebot.types.Message) -> None:
    """
    Ввод максимального расстояния от отеля до центра города с проверкой корректности
    """
    global max_distance
    ...
    if max_distance.isdigit():
        ...
        bot.send_message(message.chat.id, 'Расстояние должно быть положительным целым числом')
        bot.register_next_step_handler(message, get_max_distance)
...
def hotels_search(message: telebot.types.Message) -> None:
    ...
    bestdeal_search(message, hotels)


def bestdeal_search(message: telebot.types.Message, aux_hotels: dict):
    """
    Обработка информации из базы данных в соответствии с максимальной ценой проживания и
    максимальным расстоянием от отеля до центра города
    """
    hotels = {'data': {'body': {'searchResults': {'results': []}}}}
    hotels['data']['body']['header'] = aux_hotels['data']['body']['header']
    for i_hotel in aux_hotels['data']['body']['searchResults']['results']:
        if float(i_hotel['landmarks'][0]['distance'][:-3].replace(',', '.')) > max_distance:
            break
        else:
            if 'ratePlan' in i_hotel and i_hotel['ratePlan']['price']['exactCurrent'] <= max_price:
                hotels['data']['body']['searchResults']['results'].append(i_hotel)
    if len(hotels['data']['body']['searchResults']['results']) > hotels_number:
        hotels['data']['body']['searchResults']['results'] =\
            hotels['data']['body']['searchResults']['results'][:hotels_number]
...
def hotels_output(message: telebot.types.Message, hotels: dict) -> None:
    ...
    """
    global history_record
    history_record += ' bestdeal\n{city}\nMax цена: {price}\tMax расстояние от центра: {distance}'.format(
        city=hotels['data']['body']['header'],
        price=max_price,
